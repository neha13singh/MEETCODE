...
export default function LoginPage() {
  ...
  return (
    <div className="min-h-screen bg-background text-foreground flex items-center justify-center p-4 relative overflow-hidden selection:bg-primary/30">
      <Link href="/" className="absolute top-10 right-10 z-50 p-2 text-zinc-500 hover:text-white transition-all hover:scale-110 active:scale-95">
        <X className="w-8 h-8" />
      </Link>
      ...
      <div className="w-full max-w-md glass-card p-10 border border-primary/10 relative z-10 animate-in fade-in zoom-in duration-700 cyber-shape shadow-2xl shadow-primary/5">
        <div className="text-center mb-10">
          ...
          <h2 className="text-4xl font-black tracking-tighter italic mb-3">SIGN_IN</h2>
          <p className="text-zinc-500 font-mono text-[10px] tracking-widest uppercase font-bold">PLEASE ENTER YOUR DETAILS</p>
        </div>
        ...
        <form className="space-y-8" onSubmit={handleSubmit}>
          ...
          <div className="space-y-6">
            <div className="space-y-2">
              ...
              <input
                type="text"
                required
                className="w-full h-14 px-6 bg-espresso-dark border border-white/5 rounded-none text-white placeholder:text-zinc-700 focus:outline-none focus:border-primary/50 focus:glow-cyan transition-all cyber-shape font-mono text-sm"
          ...
          <div className="text-center">
            <Link href="/register" className="text-[10px] font-black tracking-widest text-zinc-600 hover:text-primary transition-colors uppercase">
              CREATE_AN_ACCOUNT
            </Link>
          </div>
        </form>
